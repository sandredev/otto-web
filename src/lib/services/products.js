...
export const createProduct = async (datosProducto) => {
    try {
        ...
        if (!datosProducto.nombre_producto || !datosProducto.precio || !datosProducto.categoria_id) {
            return {
                sucess: false,
                error: "Los campos nombre_producto, precio y categoria_id son obligatorios."
            };
        }
        ...
        .insert([
            {
                ...
                id_categoria: datosProducto.categoria_id,
                ingresdiente: datosProducto.ingrediente || null,
                estado: true,
        ...
        if (error) {
            ...
            return {    
                sucess: false,
                error: "Ocurrió un error al crear el producto. Por favor, inténtelo de nuevo."
            };
        }
...
export const getProducts = async (includeInactive = false) => {
    try {
        ...
        if (existProducts.length === 0 || existProducts === null){
            return {
                sucess: false,
        ...
        if (existError){
            return {
                success: false,
                error: "Existio un error al obtener los productos de la base de datos"
            };
        }
